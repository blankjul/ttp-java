...
public class OrderedCrossoverTest extends Operator{
	...
	public void testCrossover() {
		...
		Tour result = new TourOrderedCrossover().crossover(t1, t2, Pair.create(2,6));
		assertEquals(Tour.createFromString("[0,9,2,3,4,5,8,7,6,1]"), result);
		
		Tour result2 = new TourOrderedCrossover().crossover(t2, t1, Pair.create(2,6));
		assertEquals(Tour.createFromString("[0,1,8,7,6,5,2,3,4,9]"), result2);
		
		Tour tmp = new TourOrderedCrossover().crossover(t1, t2, Pair.create(0,6));
	...
	public void testNoExceptionInRepition() {
		...
		for (int i = 0; i < 50; i++) {
			new TourOrderedCrossover().crossover(t1,t2, rand);
		}
	}
