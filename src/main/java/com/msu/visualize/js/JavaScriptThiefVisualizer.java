...
public class JavaScriptThiefVisualizer extends AWriter<AExperiment> {



	@Override
	protected void write_(AExperiment experiment, OutputStream os) throws IOException {
		...
		for (IProblem problem : experiment.getProblems()) {
			...
			for(IAlgorithm algorithm : experiment.getAlgorithms()) json.writeString(algorithm.toString());
			json.writeEndArray();
			json.close();
			...
			// add for all the results the colored and the non colored version to the scatter plot
