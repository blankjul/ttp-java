...
import com.msu.algorithms.HillClimbingOnPacking;
import com.msu.algorithms.OnePlusOneEA;
import com.msu.algorithms.util.PackingListLocalSearch;
...
public class FinalExperiment2 extends AExperiment {
				...
				public void handle(ProblemFinishedEvent event) {
					...
					for (int i = 0; i < event.getNumOfRuns(); i++) {
						...
						for (IAlgorithm a : experiment.getAlgorithms()) {
							...
							for (Solution s : set) {
								if (new SolutionDominator().isDominating(single, s)) isDominating++;
								if (new SolutionDominator().isDominating(s, single)) isDominated++;
							}
							
							pw.printf("%s,%s,%s,%s,%s,%s\n", event.getProblem(), a, i, set.size(), isDominating, isDominated);
	...
	protected void setAlgorithms(List<IAlgorithm> algorithms) {
		...
		*/
		...
		algorithms.add(NSGAIIFactory.createNSGAIIBuilder("NSGAII-[OPT-OPT]-[NO-SPX]-[NO-BF]", builder).setName("OPT-OPT-NO-SPX").create());
		...
		algorithms.add(NSGAIIFactory.createNSGAIIBuilder("NSGAII-[OPT-OPT]-[NO-HUX]-[SWAP-BF]", builder).setName("OPT-OPT-NO").create());
		algorithms.add(NSGAIIFactory.createNSGAIIBuilder("NSGAII-[OPT-OPT]-[NO-HUX]-[NO-BF]", builder).setName("OPT-OPT-NO-LOCAL").create());
		
	/*	class SymmetricMutation extends AbstractMutation<List<Integer>> {

			@Override
			protected void mutate_(List<Integer> element, Random rand) {
		...
		b.setCrossover(new TTPCrossover(new NoCrossover<>(), new HalfUniformCrossover<>()));
		b.setMutation(new TTPMutation(new SymmetricMutation(), new BitFlipMutation()));
		b.setPopulationSize(200);
		b.setT(10);
